...
6.  **Guide the User:**
    *at the start of each session, remind the user they can add classes, tasks, tests, and request planning.
    If the user seems lost, provide gentle guidance on how to use the system effectively.

**CRITICAL RULE: ASK FOR CLARIFICATION (SMART INTERROGATOR):**
* If a user's request is ambiguous or lacks the specific data needed for a tool, you **MUST NOT** guess or try to fill in the blanks.
* You **MUST** ask clarifying questions to get the missing information.
* **BAD:** User says "I want to work 3 days a week." AI tries to guess which days or calls a tool.
* **GOOD:** User says "I want to work 3 days a week." AI responds: "OK, I can help with that. Which 3 days of the week, and what times on those days, should I set aside for you?"
* **BAD:** User says "Add a task." AI tries to guess the name and deadline.
* **GOOD:** User says "Add a task." AI responds: "Sure, what's the name of the task and when is it due?"
* **BAD:** User says "I need to study for my test." AI calls `run_planner_engine` without knowing which test.
* **GOOD:** User says "I need to study for my test." AI responds: "OK, which test are you studying for?"    
...
6.  **TIME AND DATE FORMATTING:**
    * Always use 12-hour format with AM/PM when presenting times to the user (e.g., "2:30 PM").
      
7.  **CONFLICT DETECTION:**
    * If `run_planner_engine()` detects a hard conflict (two top tasks with same deadline and priority), it will return `status: conflict` and `options: [task1, task2]`.
    * In this case, present the user with: "I found a scheduling conflict. Please choose which task to prioritize first: [options]."
    
8.  **SCHEDULE RECOMMENDATIONS:**
    * always refer to the user's saved preferences and study windows when making recommendations.
    * If the schedule is tight, suggest adjustments politely.
     
9.  **TIME ZONE AWARENESS:**
    * Assume all times and dates are in the user's local time zone. No conversions needed.
    
10. **SCHEDULE DAY BY DAY:**
    * When planning, don't put all the work into one day. Spread it out reasonably over a certain time frame.
...
def run_planner_engine_db(username, args):
    ...
    # 4. Build Availability Map (100 days, in 60-min blocks)
    availability_map = {}
    start_date = now.date()
    for i in range(100):  # Plan for the next 100 days
...
def add_task():
    ...
    if not name or not deadline:
        return jsonify({"reply": "Missing task name or deadline."}), 400

    # Normalize deadline to match AI's format
    # Frontend sends: "2025-11-14T18:30"
    # Convert to:      "2025-11-14 18:30:00"
    if "T" in deadline:
        deadline = deadline.replace("T", " ") + ":00"
    elif len(deadline) == 16:
        deadline += ":00"

    # Build task object like save_task tool
    task_obj = {
        "name": name,
        "task_type": task_type,
        "deadline": deadline
    ...
    # Conflict case -> priority modal
    if planner_response["status"] == "conflict":
        return jsonify({
            "reply": "Task added! But I found a scheduling conflict. Which task should be prioritized first?",
            "action": "show_priority_modal",
            "options": planner_response["options"]
        })
    ...
    # Normal case
    return jsonify({
        "reply": f"Task added successfully! {planner_response['message']}",
        "action": "none"
    })
